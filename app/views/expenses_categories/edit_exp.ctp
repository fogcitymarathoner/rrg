...
<?php echo $form->create('ExpensesCategories',array('action'=>'edit_exp'));?>
	<fieldset>
...
<?php
		...
		echo $form->input('notes',array('size'=>100)); //debug($this->data);debug($current_employee); debug($employees);
		echo $form->input('employee_id',array('label'=>'Employee', 'options'=>$employees, 'selected'=>$current_employee), null, array(), '-- Select an Employee --');
