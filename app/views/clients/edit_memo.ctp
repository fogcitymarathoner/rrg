...
<?php echo $form->create('Clients',array('action'=>'edit_memo'));?>
	<fieldset>
 		<legend><?php __('Edit Memo');?></legend>
	<?php
		echo $form->input('ClientsMemo.id',array('type'=>'hidden'));
		echo $form->input('ClientsMemo.date');
		//echo $fck->fckeditor(array('ClientsMemo', 'notes'), $html->base, $this->data['ClientsMemo']['notes']); 
		echo $form->textarea('ClientsMemo.notes',array('cols'=>100,'rows'=>60));
		echo $form->input('ClientsMemo.client_id',array('type'=>'hidden'));
