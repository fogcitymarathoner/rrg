...
<?php echo $form->create('Employees',array('action'=>'add_contract_item','id'=>'addedit_contract_item','onsubmit'=>'return validateForm()'));?>
...
//debug($clientsContract);
			...
			$res .= '<ul>	';
			$res .= '<li><a href="'.$this->webroot.'employees/view_contract_items/'.$clientsContract['ClientsContract']['id'].'">Cancel</a> </li>';
			$res .= '</ul>';
			$res .= '</div>';
			echo $res;
?>
