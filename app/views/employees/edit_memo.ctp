...
<?php echo $form->create('Employees',array('action'=>'edit_memo'));?>
	<fieldset>
 		<legend><?php __($page_title);?></legend>
	<?php
		echo $form->input('EmployeesMemo.id',array('type'=>'hidden'));
		echo $form->input('EmployeesMemo.date');
		//echo $fck->fckeditor(array('EmployeesMemo', 'notes'), $html->base, $this->data['EmployeesMemo']['notes']);
		echo $form->textarea('EmployeesMemo.notes',array('cols'=>100,'rows'=>60)); 
		echo $form->input('EmployeesMemo.employee_id',array('type'=>'hidden'));
...
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('Return to Employee', true), array('action'=>'view_memos', $this->data['EmployeesMemo']['employee_id'])); ?> </li>
	</ul>
</div>
