<?php
        ...
        function unsent_receipts($opens,$webroot)
        {
        $res = '<div style=" display: block; float:left"><h2>Unsent Timecard Receipts</h2>
        ...
        $res .= '<tr '.$class. '>';
        $res .= '<td colspan="1" >'.$invoice['Invoice']['id'].'</td>';
        ...
        {
        $res .=  'NO EMPLOYEE RECIEPT EMAILED';
        }
        }
        ...
        $res .= '<td align="right" >';
        $res .= '<a href="'.$webroot.'reminders/send_timecard_receipt/'.$invoice['Invoice']['id'].'">email receipt</a></td>';
        $res .= '</tr>';
        endforeach;
        } else {
        $res .= '<tr>
        ...
        echo unsent_receipts($opens,$this->webroot);
