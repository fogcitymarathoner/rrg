...
//exit;
$commissions_total = 0 ;
echo $javascript->link('commissions_reports_view'); //debug($commissionsReportsTag);exit;?>
<div class="commissionsReportsTags view">
<h2><?php  __('NotesReportsTag');?></h2>
	<dl><?php $i = 0; $class = ' class="altrow"';?>
		...
		<dd<?php if ($i % 2 == 0) echo $class;?>>
			<?php echo date('m/d/Y',strtotime($commissionsReport['start'])).'-'.date('m/d/Y',strtotime($commissionsReport['end'])); ?>
			&nbsp;
		</dd>
...
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('Return to NotesReport', true), array('action' => 'view/'.$tag['commissions_report_id'])); ?> </li>
	</ul>
</div>
<?php 
...
?>
	...
	<tr<?php echo $class;?>>
		<td colspan=1>
			<?php 
				if(!$invoicesItemsCommissionsItem['InvoicesItemsCommissionsItem']['opening'])
				{
					echo date('m/d/Y',strtotime($invoicesItemsCommissionsItem['InvoicesItemsCommissionsItem']['Invoice']['date']));
				} else
				{
					echo date('m/d/Y',strtotime($employee['startdate']));
				}
			?>
		</td>
		<td colspan=1>
		...
		<td>
			<?php //debug($invoicesItemsCommissionsItem['InvoicesItemsCommissionsItem']);exit;
				if($invoicesItemsCommissionsItem['InvoicesItemsCommissionsItem']['opening'])
				{
					echo $invoicesItemsCommissionsItem['InvoicesItemsCommissionsItem']['description'];
				} else
				{
...
?>
	...
	<tr<?php echo $class;?>>
		<td>
			<?php 
				echo $invoicesItemsCommissionsItem['CommissionsPayment']['number'];
			?>
		</td>
		<td>
			<?php 
