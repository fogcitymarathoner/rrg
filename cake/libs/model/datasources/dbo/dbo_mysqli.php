 ...
 */
	function connect() {
		...
		$this->connection = mysql_connect($config['host'], $config['login'], $config['password'], $config['database'], $config['port'], $config['socket']);
 ...
 */
	function value($data, $column = null, $safe = false) {
		...
		switch ($column) {
			...
			default:
				$data = "'" . mysql_real_escape_string($this->connection, $data) . "'";
			break;
		}
 ...
 */
	function resultSet(&$results) {
		...
		$j = 0;
		while ($j < $numFields) {
			$column = mysql_fetch_field_direct($results, $j);
